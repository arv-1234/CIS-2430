...
public class ePortfolio {
    ...
    public void BuyMutualFund(Scanner input) { // Method for buying mutual funds
        ...
        for (int i = 0; i < Investments.size(); i++) { // if the symbol entered is part of an existing mutual fund buy
            ...
            if (Investments.get(i).getSymbol().equalsIgnoreCase(mutualSymbol)) {
                ...
                Investments.get(i).setPrice(existPrice); // set price
                setQuant = Investments.get(i).getQuantity() + existQuantity; // calculate new quantity
                Investments.get(i).setQuantity(setQuant); // set new quantity
                existBook = Investments.get(i).getBookValue() + (existPrice * existQuantity); // calculate new book value
                Investments.get(i).setBookValue(existBook); // set new book value
                System.out.println(Investments.get(i));// print the investment
                return;
            }
        }
    ...
    public void BuyStock(Scanner input) { // method for buying stocks
        ...
        for(int i = 0; i < keyWord.length; i++){ // add individual words of name to hash map
            String word = keyWord[i].toLowerCase(); // holds individual word of name 
            if(index.containsKey(word)){
                indexValue = index.get(word);
                indexValue.add(indexVal);
            }
        ...
        System.out.println("Hash map " + index); // hash map is printed 
    ...
    public void sell(Scanner input) {
        ...
        for (int i = 0; i < Investments.size(); i++) { // if stocks array is not empty check if symbol is present in the
                                                       // arraylist
            ...
            else if (Investments.get(i).equals(new MutualFund(symbol, " ", 0, 0, 0))) { // if element in arraylist does
                                                                                        // not match symbol then print
    ...
    public void sellMutual(Scanner input, Investment mutualFund) { // Method for selling mutual funds
        ...
        if (mutualFund.getQuantity() == sellMQuant) { // if the quantity the user enters is equal to the quantity of the
            ...
            String mfName[] = nameMutual.split(" ");
            
            for(int i = 0; i < mfName.length; i++){
                String word = mfName[i].toLowerCase();

                ArrayList<Integer>indexVal = index.get(word);

                for(int j = 0; j < indexVal.size(); j++){

                    if(indexVal.get(j) > i){
                        indexVal.set(j, indexVal.get(j) - 1);
                        index.put(word, indexVal);
                    }
                    else if(indexVal.get(j) == i){
                        indexVal.remove(j);
                        index.put(word, indexVal);
                    }
                }
        ...
        try {
            ...
            while (fileScan.hasNextLine()) {
                ...
                if (info[0].equalsIgnoreCase("Stock")) {
                    Stock newStock = new Stock(info[1], info[2], Integer.parseInt(info[3]),
                    ...
                    MutualFund newMutual = new MutualFund(info[1], info[2], Integer.parseInt(info[3]),
                            Double.parseDouble(info[4]), Double.parseDouble(info[5]));

                    Investments.add(newMutual);
                    System.out.println(newMutual);
